...
class VI_WNOTIFICATION_F_Frontend_Notify {
	...
	public function product_html() {
        if ( apply_filters('woonotification_get_product_nonce',1) && !check_ajax_referer( 'viwn_nonce', 'nonce', false )) {
            echo wp_json_encode( [] );
            die;
        }
	...
	protected function get_product() {
		...
		$archive_page                   = $this->settings->archive_page();
		$mask_customer_info             = $this->settings->get_params( 'mask_customer_info' );	
		...
		if ( $archive_page > 0 ) {
			...
			switch ( $archive_page ) {
				...
				case 2:
					...
					break;
				case 4:
					$viewed_products = ! empty( $_COOKIE['woocommerce_recently_viewed'] ) ? (array) explode( '|', wc_clean( wp_unslash( $_COOKIE['woocommerce_recently_viewed'] ) ) ) : array();// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	function init_scripts() {
        ...
        wp_enqueue_style( 'woo-notification', VI_WNOTIFICATION_F_CSS . 'woo-notification' . $src_min . '.css', array(), VI_WNOTIFICATION_F_VERSION );
        wp_enqueue_script( 'woo-notification', VI_WNOTIFICATION_F_JS . 'woo-notification' . $src_min . '.js', array( 'jquery' ), VI_WNOTIFICATION_F_VERSION, true );
		if ( $this->settings->get_background_image() ) {
			wp_enqueue_style( 'woo-notification-templates', VI_WNOTIFICATION_F_CSS . 'woo-notification-templates' . $src_min . '.css', array(), VI_WNOTIFICATION_F_VERSION );
		}
				...
				if ( is_array( $names ) && count( $names ) ) {
					if($mask_customer_info) {
						$names = array_map( function ($name)  {
							return base64_encode($this->mask_random($name));
						}, $names );
					}else {
					...
					if ( is_array( $cities ) && count( $cities ) ) {
						if($mask_customer_info) {
							$cities = array_map( function ($city)  {
								return base64_encode($this->mask_random($city));
							}, $cities );
						}else {
							$cities = array_map( 'base64_encode', $cities );
						}
						$options_array['cities'] = $cities;
					}
					$country = $this->settings->get_virtual_country();
					if($mask_customer_info) {
						$country = $this->mask_random($country);
					}
					$options_array['country'] = $country;
				}
			} else {
	...
	public function mask_random($text)
	{
		...
		$first_two = substr($text, 0, 2);
		$last_one = substr($text, -1);
		$masked_middle = str_repeat('*', $length - 3);

		$text_return = $first_two . $masked_middle . $last_one;
		return $text_return;
	}
